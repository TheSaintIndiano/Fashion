...
class Modelkeras:
    ...
    def train(self, embedding_type):
        ...
        # Transforms each text in texts to a sequence of TFIDF vectors.
        train_sequences = self.tokenizer.texts_to_matrix(self.x_train, mode='tfidf')
        val_sequences = self.tokenizer.texts_to_matrix(self.x_val, mode='tfidf')
        test_sequences = self.tokenizer.texts_to_matrix(self.x_test, mode='tfidf')

        # [samples, time steps, features]
        train_sequences = np.reshape(train_sequences, (train_sequences.shape[0], 1, train_sequences.shape[1]))
        val_sequences = np.reshape(val_sequences, (val_sequences.shape[0], 1, val_sequences.shape[1]))
        test_sequences = np.reshape(test_sequences, (test_sequences.shape[0], 1, test_sequences.shape[1]))
        ...
        model.fit(train_sequences, self.y_train,
                  batch_size=self.batch_size,
                  epochs=15,
                  validation_data=(val_sequences, self.y_val),
                  verbose=2, callbacks=callbacks_list)

        score, acc = model.evaluate(test_sequences, self.y_test,
                                    batch_size=self.batch_size)
    ...
    def sample(self, embedding_type):
        ...
        test_sequences = tokenizer.texts_to_matrix(self.x_test, mode='tfidf')
        test_sequences = np.reshape(test_sequences, (test_sequences.shape[0], 1, test_sequences.shape[1]))

        # Setting checkpoint & early stopping
        checkpoint_path = os.path.join(os.path.join(self.args.checkpoint_dir, embedding_type),
                                       'model_best_weights.11-0.9809.hdf5')
        ...
        # End Time *******
        print('Time per prediction : {}'.format((end - start) / test_sequences.shape[0]))
...
if __name__ == '__main__':
    embedding_type = 'tfidf_tokenizer'
